...
export const billingApi = {
    ...
    async updateThreshold(threshold: number): Promise<ClientSettingsApiResponse> {
        try {
            const response = await api.put('/billing/settings/threshold', {
    ...
    async updateAutoRechargeEnabled(enabled: boolean): Promise<ClientSettingsApiResponse> {
        try {
            const response = await api.put('/billing/settings/auto-recharge-enabled', {
            ...
            if (response.data && typeof response.data === 'object') {
                if ('success' in response.data) {
                    ...
                    return { success: false, data: {} as any, error: apiResponse.error || { code: 'UPDATE_SETTINGS_FAILED', message: 'Invalid response' } };
                } else if ('allow_auto_recharge' in response.data) {
                    return { success: true, data: response.data as ClientSettingsResponse };
                }
            }
            return { success: false, data: {} as any, error: { code: 'UPDATE_SETTINGS_FAILED', message: 'Invalid response structure' } };
            ...
            const errorMessage = error.response?.data?.error?.message || error.response?.data?.detail || error.message || 'Failed to update auto-recharge enabled';
            return { success: false, data: {} as any, error: { code: 'UPDATE_SETTINGS_FAILED', message: errorMessage } };
        }
    }
};